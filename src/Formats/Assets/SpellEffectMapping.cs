...
/// does not include an effect type field — each spell had a hardcoded handler per spell.
/// This mapping reconstructs that relationship from game documentation and the FreeAlbion reference.
///
/// Spell ID formula: ID = school * 30 + offset + 1
/// School 0 = DjiKas, 1 = DjiKantos, 2 = DjiSchikar (Iskai), 3 = DjiOblida, 4 = DjiKura, 5 = DjiMadis
///
/// DEVIATION: Numeric SpellId construction used deliberately — the Base.Spell enum uses internal
/// asset-data identifiers that do not currently match the in-game spell names in these comments.
/// TODO: Verify all spell ID assignments against actual game asset data and replace with
/// SpellId.From(Base.Spell.X) once the correct Spell-enum-to-in-game-name mapping is confirmed.
/// </summary>
