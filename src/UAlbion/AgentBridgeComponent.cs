...
public class AgentBridgeComponent : Component, IDisposable
{
    ...
    public AgentBridgeComponent(bool openBrowser = true)
    {
        ...
        return cmd switch
        {
            ...
            _ => Error("unknown_command",
                $"Unknown command '{cmd}'. Available: ping, quit, get_scene, get_party, get_inventory, get_map, get_time, get_npcs, " +
                 "get_combat, select_combat_action, select_combat_target, equip_item, save_game, load_game, teleport, load_map, get_maps, get_position, " +
    ...
    string GetPosition()
    {
        ...
        if (state == null || !state.Loaded)
            return Error("not_ready", "No game loaded.");
        return Ok(new { x = state.PartyX, y = state.PartyY, direction = state.PartyDirection.ToString() });
    }

    static List<object>? _mapsCache;

    static readonly int[] CutsceneMaps = [165, 166, 302, 303, 304, 305];

    string GetMaps()
    {
        if (_mapsCache != null)
            return Ok(new { maps = _mapsCache });

        var assets = TryResolve<IAssetManager>();
        if (assets == null)
            return Error("not_ready", "No asset manager.");

        var maps = new List<object>();
        foreach (Base.Map m in Enum.GetValues<Base.Map>())
        {
            if ((int)m == 0) continue;

            var mapId = new MapId((int)m);
            IMapData data;
            try { data = assets.LoadMap(mapId); }
            catch { continue; }

            if (data == null) continue;

            bool cutscene = Array.IndexOf(CutsceneMaps, (int)m) >= 0;
            string type = data switch
            {
                MapData3D => "3D",
                MapData2D { MapType: MapType.TwoDOutdoors } => "2DOutdoors",
                _ => "2D"
            };
            ...
            {
                new { label = "Center", x = data.Width / 2, y = data.Height / 2 }
            };
...
// ── Action commands ───────────────────────────────────────────────────────
    ...
    string Teleport(JsonNode? node)
    {
        ...
        if (dirNode != null)
        {
            if (dirNode.GetValueKind() == System.Text.Json.JsonValueKind.String)
            {
                var dirStr = dirNode.GetValue<string>();
                dir = dirStr?.ToUpperInvariant() switch
                {
                    "NORTH" => 0,
                    "EAST" => 1,
                    "SOUTH" => 2,
                    "WEST" => 3,
                    _ => int.TryParse(dirStr, out var n) ? n : 0
                };
            }
            else
            {
                dir = dirNode.GetValue<int>();
            }
        }
        dir ??= (int)Direction.Unchanged;
    ...
    const string MonitorHtml = """
          ...
          ws.onmessage = e => {
            try {
              ...
              if (outer.type === 'response' && outer.payload) {
                ...
                if (inner.result && inner.result.maps) {
                  mapsCache = inner.result.maps;
                  if (mapsTabBuilt) buildMapsPane(inner.result.maps);
                }
              }
            } catch {}
            let cls = 'info', dir = '?';
        ...
        const HOTKEYS = {
          ...
          World3D: [
            ['+party_move 0 -1','↑ W',true],['+party_move 0 1','↓ S',true],
            ['+party_move -1 0','← A',true],['+party_move 1 0','→ D',true],
            ['+party_turn_rel -1','⟲ L',true],['+party_turn_rel 1','⟳ R',true],
            ['open_map','Map'],['wait','Wait'],['toggle_mlook','MLook'],
            ['set_mouse_mode MouseLook','MLook On'],['set_mouse_mode Normal','MLook Off'],
            ['inv:open Tom','Inv Tom'],
            ['+cam_rotate 0 0.03','CamP↑',true],['+cam_rotate 0 -0.03','CamP↓',true],
            ['+cursor_mode examine','Examine',true],['+cursor_mode manipulate','Manip',true],
            ['+cursor_mode talk','Talk',true],['+cursor_mode take','Take',true],
            ['push_scene MainMenu','MainMenu'],
          ],
        ...
        function fetchMaps() { sendRaw(JSON.stringify({cmd:'get_maps'})); }

        function getRegion(numId) {
          if (numId >= 100 && numId < 110) return 'Test Maps';
          if (numId >= 110 && numId < 150) return 'Jirinaar Region';
          if (numId >= 150 && numId < 175) return 'Toronto + Caves';
          if (numId >= 195 && numId < 200) return 'Extra Maps';
          if (numId >= 200 && numId < 220) return 'Nakiridaani';
          if (numId >= 230 && numId < 257) return 'Device Maker + Kounos';
          if (numId >= 260 && numId < 285) return 'Beloveno Region';
          if (numId >= 290 && numId < 300) return 'Test Maps 2';
          if (numId >= 300 && numId < 306) return 'Toronto Spaceship';
          if (numId >= 310 && numId < 314) return 'Kenget Prison';
          if (numId >= 320 && numId < 400) return 'Misc';
          return 'Other';
        }

        function buildMapsPane(maps) {
          const panesEl = document.getElementById('hpanes');
          let pane = panesEl.querySelector('[data-pane="Maps"]');
          if (!pane) {
            pane = document.createElement('div');
            pane.className = 'hpane';
            pane.dataset.pane = 'Maps';
            panesEl.appendChild(pane);
          }
          pane.innerHTML = '';

          if (!maps || !maps.length) {
            pane.innerHTML = '<span style="color:#888;padding:4px;">No maps loaded.</span>';
            return;
          }

          const grouped = {};
          for (const m of maps) {
            const region = getRegion(m.numId);
            if (!grouped[region]) grouped[region] = [];
            grouped[region].push(m);
          }

          for (const [region, regionMaps] of Object.entries(grouped)) {
            const section = document.createElement('div');
            section.style.cssText = 'margin-bottom:6px;';

            const header = document.createElement('div');
            header.style.cssText = 'color:#ff9800;font-size:10px;margin:2px 0;';
            ...
            select.style.cssText = 'flex:1;background:#0a1a2a;color:#e0e0e0;border:1px solid #1565c0;font-size:10px;max-width:380px;';
            ...
            for (const m of regionMaps) {
              for (let i = 0; i < m.entries.length; i++) {
                ...
                opt.value = m.id + '|' + i;
                opt.textContent = m.name + ' (' + m.type + ') \u2014 ' + m.entries[i].label;
            ...
            btn.className = 'hbtn';
            btn.style.cssText = 'border-color:#388e3c;color:#90e898;white-space:nowrap;';
            btn.textContent = '\u25B6 Teleport';
            btn.onclick = () => {
              const parts = select.value.split('|');
              const mapId = parts[0];
              const entryIdx = parseInt(parts[1]);
              const m = regionMaps.find(x => x.id === mapId);
              if (m && m.entries[entryIdx]) {
                const entry = m.entries[entryIdx];
                sendRaw(JSON.stringify({cmd:'teleport', map: mapId, x: entry.x, y: entry.y, direction: 'South'}));
              }
            };
        ...
        (function buildHotkeyTabs() {
          ...
          for (const [mode, actions] of Object.entries(HOTKEYS)) {
            ...
            if (mode === 'Characters' || mode === 'Equip' || mode === 'Combat' || mode === 'Merchant' || mode === 'Sequences' || mode === 'AutoCombat' || mode === 'Maps') {
            ...
            tab.onclick = () => {
              ...
              if (mode === 'Maps' && !mapsTabBuilt) { mapsTabBuilt = true; fetchMaps(); }
            };
            first = false;
          }
        })();
