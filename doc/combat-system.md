...
**DEVIATION:** Exact adjacency mask not confirmed; Chebyshev distance ≤ 1 used as approximation.
...
XP is split equally among living party members.
...
| # | Area | Deviation |
|---|---|---|
| D1 | Monster stat randomisation | ±5% attribute randomisation not implemented (`IEffectiveCharacterSheet` is read-only). |
| D2 | Boss crit immunity | Boss immunity flag approximated by `CharacterType.Monster` check. |
| D3 | Adjacency mask | Chebyshev ≤ 1 replaces exact original adjacency bitmask. |
| D4 | Spell/magic items | Stub — `SelectSpellDialog` not yet implemented (Bug #21). |
| D5 | Apres-combat text | SystemText IDs for victory message not confirmed; hardcoded English strings used as placeholder. |
